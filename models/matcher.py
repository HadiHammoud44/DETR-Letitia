...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_superclass: float = 1, cost_coordinates: float = 1, cost_radiomics: float = 1):
        """Creates the matcher
        ...
        assert (cost_superclass != 0 or cost_coordinates != 0 or cost_radiomics != 0), "all costs cant be 0"
    ...
    def forward(self, outputs, targets):
        ...
        # NOTE: Due to empty PET features in some samples, matching is done excluding PET radiomics
        half_radiomics_dim = out_radiomics.shape[1] // 2
        out_radiomics = out_radiomics[:, :half_radiomics_dim]
        tgt_radiomics = tgt_radiomics[:, :half_radiomics_dim]
